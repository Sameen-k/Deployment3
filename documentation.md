## PURPOSE
The purpose of this deployment was to deploy an automated elastic deployment. This application will be deployed automatically to AWS Elastic Beanstalk directly after the build and test stages.
## SYSTEM DIAGRAM 
...
3. Login to Jenkins and begin to setup a new branched pipeline. (GitHub Token will be needed. see next step)
...
9. Followed instructions for the installation of AWS EB CLI onto the ec2 instance
...
*make sure to include the backslash at the end of the URL otherwise the webhook won't be successfully generated*

10. After creating the webhook, return back to the repository on GitHub, and make a small change to the HTML files of the application files in the repository. I chose to alter the text "Website" to "WEBSITE"
...
## ISSUES
One issue occurred during the AWS EB CLI install onto the ec2 instance. To set up AWS EB CLI, after setting up the Jenkins user on the ec2 terminal, the next step is to run the following command 
...
However, this command did not run and the terminal response was to try to run the following command to install pip:

``sudo apt install pip``

After running the above command, the pip installation still failed and the reason was that the command was being run within the Jenkins user. To get out of the Jenkins user, the terminal page was just refreshed, and then the pip install command was run 
...
## OPTIMIZATION
A way to improve this deployment would be to automate the Jenkins set up through a simple script as well automating the general setup of the ec2 by creating a script to automatically download the necessary version of Python as well as the unzip application and pip, etc.
